...
class ImageFolder(torch.utils.data.Dataset):
    _repr_indent = 4
    def __init__(self, data_dir, data_list, secret_len=100, resize=256, transform=None, transform_mode='random_resized_crop', **kwargs):
        super().__init__()
        if transform is not None:
            self.transform = transform
        elif transform_mode == 'short_side_crop':
            # 짧은 변을 resize로 맞추고 CenterCrop → 비율 보존
            self.transform = transforms.Compose([
                transforms.Resize(resize),          # 짧은 변 → resize (비율 유지)
                transforms.CenterCrop(resize),       # 정방형 crop
            ])
        elif transform_mode == 'resize':
            # 원 논문 KL-f8 설정: 단순 resize (비율 무시)
            self.transform = transforms.Resize((resize, resize))
        else:  # default: random_resized_crop

...
def trainer_settings(config, output_dir):
    ...
    # logger
    logger = dict(target='pytorch_lightning.loggers.CSVLogger', params={'name': 'flux512', 'save_dir': output_dir})
...
def app(args):
    ...
    # trainer (PL2 호환: gpus → devices + accelerator)
    n_gpus = args.gpus
    strategy = 'ddp_spawn' if n_gpus > 1 else 'auto'
    trainer_kwargs = dict(devices=n_gpus, accelerator='gpu', strategy=strategy, precision=32)
